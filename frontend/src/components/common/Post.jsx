...
const Post = ({ post, setModelOpen, ismodelOpen }) => {
    ...
    mutationFn: async () => {
      try {
        const response = await axios.delete(
          `http://twitter-clone-f64h.onrender.com/api/posts/${post._id}`,
    ...
    mutationFn: async () => {
      try {
        const response = await axios.post(
          `http://twitter-clone-f64h.onrender.com/api/posts/like/${post._id}`,
          {},
          { withCredentials: true }
        );
        return response.data;
    ...
    mutationFn: async () => {
      try {
        ...
        if (post._id) {
          ...
          url = `http://twitter-clone-f64h.onrender.com/api/posts/comment/${post._id}`;
        } else if (username) {
          // If username exists, we assume it's a profile page and add logic for handling profile comment
          url = `http://twitter-clone-f64h.onrender.com/api/posts/profile/${username}/comment/${post._id}`;
        }
        const response = await axios.post(
          url,
  ...
  const { mutate: deleteAPost, isPending: isDeletingApost } = useMutation({
    mutationFn: async ({ userId, commentId, postId }) => {
      try {
        const response = await axios.delete(
          `http://twitter-clone-f64h.onrender.com/api/posts/${postId}/comments/${commentId}`,
          {
